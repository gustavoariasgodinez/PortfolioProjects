...
ORDER BY total_cases DESC; -- You can immediately notice that we have NULL values
...
SELECT continent
	  ,COUNT(*) AS N_rows
FROM PortfolioProject..CovidDeaths
GROUP BY continent; --As you can see here, there are 8,812 NULL values associated to the 'continent' column.
...
SELECT MAX(CAST(total_cases AS INT)) AS TotalCumulativeCases
...
FROM PortfolioProject..CovidDeaths
WHERE total_cases IS NOT NULL AND --Removing null values
	  total_deaths IS NOT NULL AND --Removing null values
